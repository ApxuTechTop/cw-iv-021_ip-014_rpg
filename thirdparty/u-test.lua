...
local function red(str)
    return grey and str or "\27[1;31m" .. str .. "\27[0m"
end
local function blue(str)
    return grey and str or "\27[1;34m" .. str .. "\27[0m"
end
local function green(str)
    return grey and str or "\27[1;32m" .. str .. "\27[0m"
end
local function yellow(str)
    return grey and str or "\27[1;33m" .. str .. "\27[0m"
end
...
local function run_test(test_suite, test_name, test_function, ...)
    ...
    log(string.format("%s %s %d sec", is_test_failed and fail_tag or ok_tag, full_test_name, os.difftime(stop, start)))
...
local default_start_up = function()
end
local default_tear_down = function()
    collectgarbage()
end
...
local function handle_new_test(suite, test_name, test_function)
    ...
    if info.nparams == nil and string.sub(test_name, #test_name - 1, #test_name) ~= "_p" or info.nparams == 0 then
...
local function lookup_test_with_params(suite, test_name)
    ...
    if all_test_cases[suite_name] and all_test_cases[suite_name][test_name] then
        return function(...)
            run_test(suite, test_name .. "(" .. stringize_var_arg(...) .. ")", all_test_cases[suite_name][test_name],
                     ...)
        end
    else
        local full_test_name = test_pretty_name(suite_name, test_name)
...
local function new_test_suite(_, name)
    ...
    setmetatable(test_suite, {__newindex = handle_new_test, __index = lookup_test_with_params})
    return test_suite
end
